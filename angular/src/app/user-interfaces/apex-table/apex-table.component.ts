...
@Component({
    ...
    styleUrls: ['./apex-table.component.scss'],
})
export class ApexTableComponent implements OnInit, OnDestroy {
    /**Properties******************************************/
    ...
    handleArrayChanges() {
        let debug = Debug('ApexTable:handleArrayChanges');
    ...
    setColumns() {
        let debug = Debug('ApexTable:setColumns');
        ...
        if (this.array) {
            ...
            if (this.paths) {
                pathsToShow.push(...this.paths);
            } else if (this.pathsCsv) {
                pathsToShow.push(
                    ...this.pathsCsv
                        .split(',')
            ...
            for (let path of pathsToShow) {
                ...
                if (!businessObjectsByPath[parentPath]) {
                    ...
                    businessObjectsByPath[parentPath] = currentBusinessObject;
                }
            }
            debug('businessObjectsByPath', businessObjectsByPath);
                ...
                if (propertyName == '*') {
                    ...
                    if (!property && propertyName == 'id') {
                        ...
                        for (let relationshipName in businessObject.relationships) {
                            let relationship =
                                businessObject.relationships[relationshipName];
                            debug('relationship', relationship);
                            if (relationship.foreignKey == propertyName) {
                    ...
                    if (!property) {
                        property = {
                            displayName: changeCase.capitalCase(propertyName),
                            name: propertyName,
                            type: {
                                name: 'string',
                            },
                    ...
                    debug('property to add to derivedColumnLabels:', property);
                    derivedColumnLabels.push(property.displayName);
                    this.columnTypes.push(property.type.name);
                    if (parentPath !== '') {
                        this.columnPaths.push(parentPath + '.' + property.name);
                    } else {
                        this.columnPaths.push(property.name);
                    }
                }
...
*/
    ...
    setDefaultRoutePrefix() {
        let debug = Debug('ApexTable:setDefaultRoutePrefix');
    ...
    setIdPropertyName() {
        let debug = Debug('ApexTable:setIdPropertyName');
    ...
    listenForChanges() {
        let debug = Debug('ApexTable:listenForChanges');
    ...
    renderRows() {
        let debug = Debug('ApexTable:renderRows');
    ...
    itemUrl(item: any) {
        ...
        if (this.routeFunction) {
            ...
            url = url + item[this.idPropertyName];
        }
        return url;
    }
    ...
    handleRowSelection() {
        let debug = Debug('ApexTable:handleRowSelection');
    ...
    clearSelection() {
        let debug = Debug('ApexTable:clearSelection');
    ...
    handleSelectedRowsChange() {
        let debug = Debug('ApexTable:handleSelectedRowsChange');
