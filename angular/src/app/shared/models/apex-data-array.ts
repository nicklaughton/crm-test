...
export class ApexDataArray extends ApexFormObjectArray {
	...
	protected _deserialize(objects: Array<any>, context?: any) {
		...
		for (let i = 0; i < objects.length; i++) {
			...
			if (objects[i] instanceof this._classReference) {
				...
				this._push(new this._classReference(objects[i], this._copyOptions(), context ?? this._context));
			}
	...
	hasPendingChanges(): boolean {
		...
		for (let item of this) {
			if (item && item['hasPendingChanges'] && item['hasPendingChanges']()) {
				hasPendingChanges = true;
			}
		}
		if (this._pendingDeletions.length > 0) hasPendingChanges = true;
	...
	async add(object: any, context?: any): Promise<any> {
		...
		const newObject =
			object instanceof this._classReference
				? object
				: new this._classReference(object, this._copyOptions(), context ?? this._context);
