...
export abstract class ApexDataObject<Q = any> extends ApexFormObject {
	...
	protected _deserialize(object: any, context?: any, isConstructing?: boolean) {
		...
		for (let name in object) {
			...
			if (typeof object[name] != 'function' && !name.startsWith('_')) {
				...
				if (
					...
					object[name] &&
					!(object[name] instanceof Date) &&
					!this._isProperty(name)
				) {
		...
		const stringifiedChanges = JSON.stringify(this._pendingChanges);

		if (
			stringifiedChanges != '{}' &&
			!isConstructing &&
			!(this._options.foreignKeyObject && JSON.stringify(this._options.foreignKeyObject) == stringifiedChanges)
		) {
	...
	async read(context?: any): Promise<void> {
		...
		if (!this._repository) {
			...
			this._options.read === 'Never' ||
			(this._options.readAutomatically === false && this._options.readOnDemand === false)
		) {
	...
	async save(saveTime?: number, context?: any): Promise<void> {
		...
		if (saveTime > this._lastSaveTime) {
			...
			if (!this._repository) {
				...
				const stringifiedChanges = JSON.stringify(pendingChanges);

				// skip if there are no changes or the changes are just creating the foreignKeyObject
				if (
					stringifiedChanges !== '{}' &&
					!(
						!id &&
						this._options.foreignKeyObject &&
						JSON.stringify(this._options.foreignKeyObject) == stringifiedChanges
					)
				) {
					if (id) {
						this.saving = true;
	...
	async applyInclude(include: any) {
		await this.applyFilter({ include });
	}

	async applyWhere(where: any) {
		await this.applyFilter({ where });
	}
	...
	hasPendingChanges(): boolean {
		debug('_pendingChanges', this._pendingChanges);
		return this._pendingChanges && Object.keys(this._pendingChanges).length > 0;
	}

	private _isProperty(propertyName: string): boolean {
		const properties = this.getMetadata('properties');
		if (!properties || !properties.length) return false;
		const property = properties.find((prop) => propertyName == prop.name);
		return !!property;
	}
	...
	protected static async _findById<Q>(id: any, options: ApexDataOptions, context?: any): Promise<Q> {
		...
		const result = await this._staticRepository.findById(id, filter, context);
		if (result) {
			const objectClass = this.getObjectClass();
			return new objectClass(result?.toJSON ? result.toJSON() : result, options, context);
		} else return null;
	}

	protected static async _create<Q>(data: Object, options?: ApexDataOptions, context?: any): Promise<Q> {
		const result = await this._staticRepository.create(data, context);
	...
	static async createWithChildren<Q extends ApexDataObject<any>>(
		...
		let basicObject = {};
		for (let nam in data) {
			if (typeof data[nam] != 'object' || data[nam] instanceof Date) basicObject[nam] = data[nam];
		}
	...
	protected static async _findOne<Q>(options: ApexDataOptions, context?: any): Promise<Q> {
		const filter = this._prepareFilter(options);
		const result = await this._staticRepository.findOne(filter, context);
		if (result) {
			const objectClass = this.getObjectClass();
			return new objectClass(result?.toJSON ? result.toJSON() : result, options, context);
		} else return null;
	...
	static async deleteAll(where: object, context?: any): Promise<number> {
		if (where && Object.keys(where).length == 1 && Object.keys(where)[0] == 'where')
	...
	protected static async _createAll<Q>(data: Object[], options?: ApexDataOptions, context?: any): Promise<Q> {
		const result = await this._staticRepository.createAll(data, context);
		const arrayClass = this.getArrayClass();
		return new arrayClass(result?.toJSON ? result.toJSON() : result, options, context);
	}
